...
def analyze_confidence(facial_analysis, audio_analysis):
    ...
    positive_emotions = sum(
        facial_analysis.get(emotion, 0) for emotion in ["happy", "confident"]
    )
    negative_emotions = sum(
        facial_analysis.get(emotion, 0) for emotion in ["fear", "nervous", "sad"]
    )
    ...
    if confidence_score > 0.7:
        ...
        feedback["recommendations"].append(
            "Practice power posing before interviews and maintain eye contact"
        )

    return feedback


def analyze_engagement(facial_analysis, audio_analysis, _text_analysis):
    """
    ...
    # Extract only emotion data from facial analysis
    emotion_data = {}
    for key, value in facial_analysis.items():
        # Skip non-emotion entries like 'emotion_sums' or 'timeline'
        if isinstance(value, (int, float)) and key not in ["emotion_sums", "timeline"]:
            emotion_data[key] = value

    # Calculate emotion variety - how many emotions are significantly present
    ...
    if speech_variation > 0.7:
        ...
        feedback["areas_for_improvement"].append(
            "Your vocal tone could use more variation"
        )
        feedback["recommendations"].append(
...
def generate_competency_feedback(facial_analysis, audio_analysis, text_analysis):
    ...
    """
    feedback = {
        "communication_clarity": analyze_communication_clarity(
